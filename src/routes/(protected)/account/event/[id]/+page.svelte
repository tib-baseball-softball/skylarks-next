      ...
      <div class="card variant-soft-success flex-grow">
        ...
        <section class="p-4 flex flex-wrap gap-1 lg:gap-2">
            {#each participationsIn as inResponse}
                <div class="chip variant-ghost-success">{inResponse?.expand?.user.first_name}</div>
            {/each}
        </section>
      ...
      <div class="card variant-soft-warning flex-grow">
        ...
        <section class="p-4">
            {#each participationsMaybe as maybeResponse}
                <div class="chip variant-ghost-warning">{maybeResponse?.expand?.user.first_name}</div>
            {/each}
        </section>
      </div>
      ...
      <div class="card variant-soft-error flex-grow">
        ...
        <section class="p-4">
            {#each participationsOut as outResponse}
                <div class="chip variant-ghost-error">{outResponse?.expand?.user.first_name}</div>
            {/each}
        </section>
      </div>
  </section>
