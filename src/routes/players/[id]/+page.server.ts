...
export const load: PageServerLoad = async ({params, fetch}) => {
  ...
  const playerPitchingStats = statsRequest.getStatisticsForPerson<PitchingStatisticsEntry>(Number(params.id), StatsType.pitching);
  const playerFieldingStats = statsRequest.getStatisticsForPerson<FieldingStatisticsEntry>(Number(params.id), StatsType.fielding);
