...
<Dialog.Root bind:open {...restProps}>
  <Dialog.Trigger class="{triggerClasses} flex gap-1" type="button" {disabled}>
    {@render triggerContent()}
  </Dialog.Trigger>
  <Dialog.Portal>
    <Dialog.Overlay
      class="fixed inset-0 z-50 bg-surface-50/80 dark:bg-surface-950/80"
    />
    <Dialog.Content
      forceMount
        ...
        {#if open}
          ...
          >
            <div class="flex gap-5 items-center mb-2">
              <Dialog.Close
                class="btn preset-tonal-surface border border-surface-500 {closeButtonClasses}"
              >
                <X />
              </Dialog.Close>
