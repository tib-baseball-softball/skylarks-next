<script lang="ts">
  ...
  async function getCurrentGamesCount(): Promise<number> {
    const response = await client.send<GamesCount>(
      `/api/dp/teams/${team.id}/gamecount?season=${season}`,
      {},
    );
  ...
  async function loadClubLeagueGroups() {
    ...
    const loadLeagueGroups = client
      .collection(Collection.LeagueGroups)
...
<form class="team-games-form" onsubmit={submitForm}>
  <div class="form-grid">
    ...
    <label class="label form-label">
      ...
      <select bind:value={season} class="select" name="season">
      ...
      <label class="label form-label">
        ...
        <select class="select" bind:value={form.bsm_league_group}>
          ...
          {#each leagueGroups as leagueGroup}
            <option value={leagueGroup.bsm_id}
              >{leagueGroup.name} ({leagueGroup.acronym}, {leagueGroup.season})</option
            >
          {/each}
        </select>
      </label>
    ...
    <p class="footer-info col-full">
      To import games automatically, Diamond Planner needs to know the league
      this team is currently playing in. The league is imported from BSM and
      changes every season (even if the actual league name is still the same).
    </p>
